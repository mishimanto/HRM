...
const Reports = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats Cards */}
      <div className="grid grid-cols-2 gap-px border border-gray-200 bg-gray-200 lg:grid-cols-6">
        {[
          ['Headcount', hrmData.headcount || 0], ['Turnover', `${hrmData.turnover_rate || 0}%`],
          ['Net payroll', `৳${Number(hrmData.payroll_total || 0).toLocaleString()}`],
          ['Expenses', `৳${Number(hrmData.expense_total || 0).toLocaleString()}`],
          ['Learning complete', `${hrmData.training_completion_rate || 0}%`], ['Pending approvals', hrmData.pending_approvals || 0],
        ].map(([label,value]) => <div key={label} className="bg-white p-4"><p className="text-xs uppercase text-gray-500">{label}</p><p className="mt-2 text-xl font-semibold text-gray-900">{value}</p></div>)}
      </div>
